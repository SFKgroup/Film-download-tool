...
def change_active():
    ...
    print('    \033[36m(0) \033[0m'+source_name[0]+' \033[2m(default)\033[0m')
...
def search(args=None):
    ...
    video_dic = search_videos(sources[source_name[active_source]],word,page)
    print(' '*(width-1),end='\r')
    ...
    for i,v in enumerate(video_dic['videos']):
        print('    \033[36m('+str(i)+') \033[0m'+v['name'])
        names.append(v['name'])
        ...
        try:
            ...
            for i in ans.split(','):
                if '-' in i:
                    ...
                    for n in range(_[0],_[1]+1):dl_list[video_names[n]] = active_video['links'][video_names[n]]
                elif i.isdigit() and 0 <= int(i) < len(video_names):dl_list[video_names[int(i)]] = active_video['links'][video_names[int(i)]]
        except:print('\033[31m输入不规范\033[0m')
